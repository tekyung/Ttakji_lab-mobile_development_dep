...
public class firebase_network : MonoBehaviour
{
    ...
    public void ListenForEvent(string sessioncode, Action<string, string> eventreceive)
    {
        ...
        {
            Debug.LogError("Firebase Error: sessioncode가 비어있습니다! 호출 위치를 확인하세요.");
            return;
        }
        ...
        if (dbRef == null)
        {
            Debug.LogError("Firebase Error: dbRef가 초기화되지 않았습니다. Initialize()를 먼저 기다려야 합니다.");
            return;
        }
        ...
        try
        {
            ...
            // 문제가 발생하는 지점: sessioncode에 특수문자가 있는지 로그로 찍어보세요.
