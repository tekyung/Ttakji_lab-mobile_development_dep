...
| `"max_deck_count"` | 숫자 | **덱에 넣을 수 있는 최대 장수.** 룰북상 `2`다. ⚠️ 아래 주의 참조 |
...
> ### ⚠️ `max_deck_count`를 빠뜨리지 마세요 (2026-08-18 추가)
>
> 이 값이 없으면 JSON 파서가 **0으로 읽습니다.** 그러면 덱 빌더에서 그 카드를
> **추가 버튼을 눌러도 아무 반응이 없습니다**(0장까지만 넣을 수 있다는 뜻이 되기 때문).
> 실제로 40장 전부에 이 값이 빠져 있어 덱 편집이 통째로 막힌 적이 있습니다.
>
> 지금은 코드가 값이 없으면 룰북 기본값 `2`로 보고 콘솔에 경고를 남기지만,
> **새 카드에는 명시적으로 적어 주세요.**
