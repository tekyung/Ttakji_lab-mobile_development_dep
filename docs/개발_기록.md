...
> 이 문서는 **2026-08-18 이전의 README 내용을 그대로 옮겨 둔 것**이다.
> 저장소 루트의 `README.md`는 "프로젝트가 무엇이고 지금 어디까지 왔는가"를 알리는 문서로 새로 썼다.
>
> ⚠️ **아래 내용 중 일부는 현재 코드와 다르다.** 당시 기록으로만 읽을 것.
> 최신 사실은 아래 문서를 본다:
> - 현재 상태·남은 일 → 루트 `README.md`
> - 상세 인수인계 → `Assets/TCG_Project/HANDOFF.md`
> - 이벤트 계약(정본) → `Assets/TCG_Project/EVENTMANAGER_CONTRACT.md`
>
> 특히 아래 UI 연동 가이드에는 **지금은 사실이 아닌 서술**이 섞여 있다:
> - `OnRequireStackResponse`는 **현재 발행처가 없다**(스택 응답은 `OnRequireCardPick`으로 처리된다)
> - `OnCardStateChange`의 실제 이름은 `OnCardStateChanged`다
> - `OnCardDiscard` / `OnCardUnstacked` / `OnCardUnbattlefield`는 선언만 있고 발행처가 없다
> - 카드 이미지 경로는 `GameDesign/card_image/...`가 아니라 **`Assets/Resources/card_image/{ELLI|VERONICA|DAINA|SONIA}/`** 다
> - 3판 2선승이 아니라 **단판제**로 운영 중이다 (`CommonConfig.json`의 `Bot_single_game`)
