...
### 내 PC에서 처음 열 때 — `.mcp.json` 한 줄

저장소에 `.mcp.json`이 들어 있어 Unity MCP(에디터를 CLI·AI로 조작)를 바로 씁니다.
다만 **`--project-path` 한 줄은 사람마다 고쳐야 합니다.**

```json
"--project-path", "<이 저장소를 받은 절대경로>"
```

`command`는 `"unity"`(PATH)라 그대로 두면 됩니다 — Unity CLI를 설치하면 PATH에 들어갑니다.

> 경로를 아예 빼면 열려 있는 모든 에디터 중에서 고르는 것으로 보입니다.
> 유니티 프로젝트를 둘 이상 열어 두면 엉뚱한 쪽에 붙을 수 있어 명시해 둡니다.
